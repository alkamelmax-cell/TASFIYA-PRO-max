...
async function handleSaveReconciliation() {
    ...
    try {
        ...
        let isRecalled = currentReconciliation.id !== undefined;
        if (isRecalled) {
            // تأكيد من المستخدم
            const confirmed = await DialogUtils.showConfirm(
...
async function generateTimeBasedReportData(reportType, dateFrom, dateTo) {
    ...
    switch (reportType) {
        ...
        case 'weekly':
            dateFormat = '%Y-W%W';
            groupBy = 'strftime("%Y-W%W", r.reconciliation_date)';
            break;
        case 'monthly':
            dateFormat = '%Y-%m';
    ...
    const query = `
        SELECT
            strftime('${dateFormat}', r.reconciliation_date) as period,
...
async function populateEditModal(data) {
    ...
    try {
        ...
        if (reconciliationIdElement) {
            reconciliationIdElement.textContent = `#${reconciliation.id}`;
...
async function populateEditFormFields(reconciliation) {
    ...
    try {
        ...
        console.log('👥 [FORM-FIELDS] تحميل الكاشيرين والمحاسبين...');
...
/**
 * Ensure cashiers and accountants are loaded in edit modal selects
 */
...
function populateEditBankReceiptsTable(bankReceipts) {
    ...
    bankReceipts.forEach((receipt, index) => {
        ...
        if (!receipt.operation_type || !receipt.atm_id) {
            ...
            if (!receipt.operation_type) receipt.operation_type = 'مدى';
            if (!receipt.atm_id) receipt.atm_id = 1; // Default to first ATM
        }
...
// Override old print functions to prevent conflicts
window.printReconciliation = printSavedReconciliation;
window.quickPrintReconciliation = quickPrintSavedReconciliation;
window.generatePDFReconciliation = generatePDFSavedReconciliation;

console.log('✅ [NEW-PRINT] نظام الطباعة الجديد تم تحميله بنجاح');
...
async function loadDetailedAtmReportFilters() {
    try {
        ...
        atms.forEach(atm => {
            const option = document.createElement('option');
            option.value = atm.id;
            option.textContent = `${atm.name} - ${atm.branch_name || 'غير محدد'} - ${atm.location || 'غير محدد'}`;
            atmSelect.appendChild(option);
        });
...
function applyTheme(theme) {
    ...
    console.log(`🎨 [SETTINGS] تم تطبيق المظهر: ${theme}`);
}
...
function initializeAutocomplete() {
    ...
    try {
        ...
        // تهيئة النص التنبؤي لحقل اسم العميل في المبيعات الآجلة
        initializePostpaidCustomerAutocomplete();

        // تهيئة النص التنبؤي لحقل اسم العميل في مقبوضات العملاء
        initializeCustomerReceiptAutocomplete();
 ...
 * تهيئة النص التنبؤي لحقل اسم العميل في المبيعات الآجلة
 */
function initializePostpaidCustomerAutocomplete() {
    console.log('📱 [AUTOCOMPLETE] تهيئة النص التنبؤي للمبيعات الآجلة...');

    autocompleteSystem.initialize('customerName', {
        minLength: 1,
        debounceDelay: 300,
        maxResults: 8,
        ...
        dataSource: async (query) => {
            try {
                ...
                console.log(`✅ [AUTOCOMPLETE] تم العثور على ${suggestions.length} اقتراح للمبيعات الآجلة`);
                return suggestions;

            } catch (error) {
                console.error('❌ [AUTOCOMPLETE] خطأ في جلب اقتراحات المبيعات الآجلة:', error);
                return [];
            }
        },
        onSelect: (value, input) => {
            console.log(`✅ [AUTOCOMPLETE] تم اختيار عميل المبيعات الآجلة: "${value}"`);

            // يمكن إضافة منطق إضافي هنا مثل جلب بيانات العميل
            // أو عرض إحصائيات سريعة
            showCustomerQuickStats(value, 'postpaid');
        }
    });

    console.log('✅ [AUTOCOMPLETE] تم تهيئة النص التنبؤي للمبيعات الآجلة');
}

/**
 * تهيئة النص التنبؤي لحقل اسم العميل في مقبوضات العملاء
 */
function initializeCustomerReceiptAutocomplete() {
    console.log('💰 [AUTOCOMPLETE] تهيئة النص التنبؤي لمقبوضات العملاء...');

    autocompleteSystem.initialize('customerReceiptName', {
        minLength: 1,
        debounceDelay: 300,
        maxResults: 8,
        placeholder: 'ابدأ كتابة اسم العميل...',
        dataSource: async (query) => {
            try {
                ...
                console.log(`✅ [AUTOCOMPLETE] تم العثور على ${suggestions.length} اقتراح لمقبوضات العملاء`);
                return suggestions;

            } catch (error) {
                console.error('❌ [AUTOCOMPLETE] خطأ في جلب اقتراحات مقبوضات العملاء:', error);
                return [];
            }
        },
        onSelect: (value, input) => {
            console.log(`✅ [AUTOCOMPLETE] تم اختيار عميل المقبوضات: "${value}"`);

            // عرض إحصائيات سريعة للعميل
            showCustomerQuickStats(value, 'receipts');
        }
    });

    console.log('✅ [AUTOCOMPLETE] تم تهيئة النص التنبؤي لمقبوضات العملاء');
}
