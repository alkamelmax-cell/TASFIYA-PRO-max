  ...
  getNextReconciliationNumber() {
    try {
      const transaction = this.db.transaction(() => {
        // Get the max number with a write lock to prevent race conditions
        const result = this.db.prepare(`
          SELECT MAX(reconciliation_number) as max_number
          FROM reconciliations
          WHERE status = 'completed' AND reconciliation_number IS NOT NULL
        `).get();
        ...
        while (true) {
          ...
          console.warn(`⚠️ [DB] الرقم ${nextNum} مستخدم بالفعل، جاري تجربة الرقم التالي`);
          nextNum++;
        }

        console.log('📊 [DB] الرقم التسلسلي التالي للتصفية:', nextNum);
        return nextNum;
      });
