...
} = require('./app/customer-code-prefix');
...
const SEQUENTIAL_GENERATED_CUSTOMER_CODE_REGEX = /^C(\d+)-(\d{6})$/;
const LEGACY_GENERATED_CUSTOMER_CODE_REGEX = /^C(\d+)-(?=[A-Z0-9]*[A-Z])[A-Z0-9]{10}(?:-X)?$/;
...
class DatabaseManager {
  ...
  createTables() {
      ...
      if (!tableExists) {
        ...
        try {
          ...
          if (hasData && hasData.count > 0) {
            this.db.exec(`
              INSERT INTO customer_receipts_new (id, reconciliation_id, customer_name, customer_code, amount, payment_type, created_at)
              SELECT id, reconciliation_id, customer_name, COALESCE(customer_code, ''), amount, payment_type, created_at FROM customer_receipts
            `);
    ...
    try {
      ...
      customerCodeTables.forEach((tableName) => {
        const columns = this.db.prepare(`PRAGMA table_info(${tableName})`).all();
        const hasCustomerCode = Array.isArray(columns)
          && columns.some((column) => String(column?.name || '') === 'customer_code');

        if (!hasCustomerCode) {
          this.db.exec(`ALTER TABLE ${tableName} ADD COLUMN customer_code TEXT DEFAULT ''`);
        }
      });
    } catch (error) {
      console.error('❌ [DB] خطأ في إضافة عمود customer_code إلى جداول العملاء:', error);
    }
    ...
    try {
      ...
      customerIdTables.forEach((tableName) => {
        const columns = this.db.prepare(`PRAGMA table_info(${tableName})`).all();
        const hasCustomerId = Array.isArray(columns)
          && columns.some((column) => String(column?.name || '') === 'customer_id');

        if (!hasCustomerId) {
          this.db.exec(`ALTER TABLE ${tableName} ADD COLUMN customer_id INTEGER`);
        }
      });
    } catch (error) {
    ...
    try {
      ...
      console.error('❌ [DB] خطأ في تهيئة رموز أكواد العملاء للفروع:', error);
    }

    try {
      this.syncCustomerRegistryFromTransactions();
    } catch (error) {
      console.error('❌ [DB] خطأ في مزامنة سجل العملاء:', error);
    }

    try {
      this.normalizeGeneratedCustomerCodes();
    } catch (error) {
      console.error('❌ [DB] خطأ في تهيئة أكواد العملاء المولدة:', error);
    }

    try {
      this.normalizeCustomerCodeBranchSegments();
    } catch (error) {
      console.error('❌ [DB] خطأ في مواءمة أكواد العملاء مع ترتيب الفروع:', error);
    }

    try {
      this.repairDuplicateCustomerCodes();
      this.relinkCustomerTransactionsByUniqueCodes();
      this.repairCustomerTransactionBranchMismatches();
    } catch (error) {
      console.error('❌ [DB] خطأ في معالجة أكواد العملاء المكررة:', error);
    }
  ...
  updateDatabaseSchema() {
    try {
      ...
      const hasBranchCustomerCodePrefix = branchColumns.some(col => col.name === 'customer_code_prefix');
      ...
      if (!hasBranchCustomerCodePrefix) {
        console.log('➕ [DB] إضافة عمود customer_code_prefix إلى جدول الفروع...');
        this.db.exec("ALTER TABLE branches ADD COLUMN customer_code_prefix TEXT DEFAULT ''");
        console.log('✅ [DB] تم إضافة عمود customer_code_prefix بنجاح');
      }
  ...
  createPerformanceIndexes() {
    try {
      const indexStatements = [
        ...
        'CREATE INDEX IF NOT EXISTS idx_manual_postpaid_customer_code ON manual_postpaid_sales(customer_code)',
        'CREATE INDEX IF NOT EXISTS idx_manual_postpaid_customer_id ON manual_postpaid_sales(customer_id)',
        'CREATE INDEX IF NOT EXISTS idx_manual_receipts_customer_code ON manual_customer_receipts(customer_code)',
  ...
  applyDataIntegrityLayer() {
    try {
      ...
      this.createUniqueIndexIfNoDuplicates(
        'idx_branches_customer_code_prefix_unique',
        `SELECT UPPER(TRIM(customer_code_prefix)) AS value, COUNT(*) AS count
         FROM branches
         WHERE TRIM(COALESCE(customer_code_prefix, '')) <> ''
         GROUP BY UPPER(TRIM(customer_code_prefix))
         HAVING COUNT(*) > 1`,
      ...
      this.createUniqueIndexIfNoDuplicates(
        ...
        `SELECT UPPER(TRIM(customer_code)) AS value, COUNT(*) AS count
         FROM customers
  ...
  insertDefaultData() {
    try {
      ...
      if (branchCount.count === 0) {
        this.db.prepare(`
          INSERT INTO branches(branch_name, customer_code_prefix, branch_address, branch_phone, is_active)
          VALUES(?, ?, ?, ?, ?)
  ...
  getTableColumns(tableName) {
    try {
      ...
      console.warn(`⚠️ [DB] تعذر قراءة أعمدة جدول ${tableName}:`, error && error.message ? error.message : error);
      return [];
    }
  }
  ...
  ensureBranchCustomerCodePrefixColumn() {
    if (this.hasTableColumn('branches', 'customer_code_prefix')) {
      return;
    }
  ...
  getFallbackCustomerCodeBranchSegment(branchId = null) {
    const normalizedBranchId = this.normalizeCustomerRegistryBranchId(branchId);
    ...
    try {
      ...
      const branchIndex = Array.isArray(branches)
        ? branches.findIndex((branch) => Number(branch?.id) === normalizedBranchId)
        : -1;
      if (branchIndex >= 0) {
        return branchIndex + 1;
      }
    } catch (error) {
      console.error('⚠️ [DB] تعذر قراءة الفروع لترقيم العملاء:', error);
    }

    return normalizedBranchId;
  }

  ensureBranchCustomerCodePrefixes() {
    this.ensureBranchCustomerCodePrefixColumn();

    const branches = this.db.prepare(
      'SELECT id, customer_code_prefix FROM branches ORDER BY id ASC'
    ).all();
    ...
    branches.forEach((branch, index) => {
      const existingPrefix = normalizeCustomerCodePrefix(branch.customer_code_prefix);
      let nextPrefix = existingPrefix;

      if (!nextPrefix || usedPrefixes.has(nextPrefix)) {
        nextPrefix = normalizeCustomerCodePrefix(`C${index + 1}`);
      }

      if (!nextPrefix || usedPrefixes.has(nextPrefix)) {
        nextPrefix = getNextCustomerCodePrefix(Array.from(usedPrefixes));
      }

      usedPrefixes.add(nextPrefix);

      if (existingPrefix !== nextPrefix) {
        updates.push({
          id: branch.id,
          customer_code_prefix: nextPrefix
        });
      }
    });
    ...
    const updateStmt = this.db.prepare(
      `UPDATE branches
       SET customer_code_prefix = ?,
           updated_at = CURRENT_TIMESTAMP
       WHERE id = ?`
    );
    const applyUpdates = this.db.transaction((rows) => {
      rows.forEach((row) => {
        updateStmt.run(row.customer_code_prefix, row.id);
      });
    });
    applyUpdates(updates);

    console.log(`✅ [DB] تم تهيئة رموز أكواد العملاء لعدد ${updates.length} فرع`);
    return { updated: updates.length };
  }

  normalizeCustomerRegistryName(value) {
    return String(value == null ? '' : value).trim();
  }

  normalizeCustomerRegistryCode(value) {
    const normalizedCode = String(value == null ? '' : value).trim().toUpperCase();
    return ['-', '–', '—'].includes(normalizedCode) ? '' : normalizedCode;
  }

  normalizeCustomerRegistryBranchId(value) {
    const numericValue = Number(value);
    return Number.isFinite(numericValue) && numericValue > 0 ? numericValue : null;
  }

  normalizeCustomerCodeBranchSegment(value) {
    const numericValue = Number(value);
    return Number.isFinite(numericValue) && numericValue > 0 ? Math.floor(numericValue) : 0;
  }

  parseGeneratedCustomerCodeBranchId(customerCode) {
    const normalizedCode = this.normalizeCustomerRegistryCode(customerCode);
    const match = normalizedCode.match(/^C(\d+)-/);
    if (!match) {
      return null;
    }

    const branchSegment = Number(match[1]);
    return Number.isFinite(branchSegment) && branchSegment >= 0 ? branchSegment : null;
  }

  parseSequentialGeneratedCustomerCode(customerCode) {
    const normalizedCode = this.normalizeCustomerRegistryCode(customerCode);
    const match = normalizedCode.match(SEQUENTIAL_GENERATED_CUSTOMER_CODE_REGEX);
    if (!match) {
      return null;
    }
    ...
    if (!Number.isFinite(branchSegment) || !Number.isFinite(sequence)) {
      return null;
    }

    return {
      branchPrefix: extractCustomerCodePrefix(normalizedCode, { allowZero: true }),
      branchSegment,
      sequence
    };
  }

  isLegacyGeneratedCustomerCode(customerCode) {
    const normalizedCode = this.normalizeCustomerRegistryCode(customerCode);
    return LEGACY_GENERATED_CUSTOMER_CODE_REGEX.test(normalizedCode);
  }

  getCustomerCodeBranchSegment(branchId = null) {
    const branchPrefix = this.getCustomerCodeBranchPrefix(branchId);
    const branchSegment = Number(String(branchPrefix || '').replace(/^C/, ''));
    return Number.isFinite(branchSegment) && branchSegment >= 0 ? branchSegment : 0;
  }

  getCustomerCodeBranchPrefix(branchId = null) {
    const normalizedBranchId = this.normalizeCustomerRegistryBranchId(branchId);
    if (!normalizedBranchId) {
      return 'C0';
    }
    ...
    try {
      ...
      const branchPrefix = normalizeCustomerCodePrefix(row?.customer_code_prefix);
      if (branchPrefix) {
        return branchPrefix;
      }
    } catch (error) {
      console.error('⚠️ [DB] تعذر قراءة رمز أكواد العملاء للفرع:', error);
    }

    const fallbackSegment = this.getFallbackCustomerCodeBranchSegment(normalizedBranchId);
    return normalizeCustomerCodePrefix(`C${fallbackSegment}`, { allowZero: true }) || 'C0';
  }

  getLegacyCustomerCodeBranchSegment(branchId = null) {
    const normalizedBranchId = this.normalizeCustomerRegistryBranchId(branchId);
    if (!normalizedBranchId) {
      return 0;
    }

    const findSegment = (branches) => {
      const branchIndex = Array.isArray(branches)
        ? branches.findIndex((branch) => Number(branch?.id) === normalizedBranchId)
        : -1;
      return branchIndex >= 0 ? branchIndex + 1 : null;
    };

    try {
      const activeBranches = this.db.prepare(
        'SELECT id FROM branches WHERE COALESCE(is_active, 1) = 1 ORDER BY id ASC'
      ).all();
      const activeSegment = findSegment(activeBranches);
      if (activeSegment) {
        return activeSegment;
      }
    } catch (error) {
      console.error('⚠️ [DB] تعذر قراءة الفروع النشطة لترقيم العملاء:', error);
    }

    try {
      const allBranches = this.db.prepare(
        'SELECT id FROM branches ORDER BY id ASC'
      ).all();
      const branchSegment = findSegment(allBranches);
      if (branchSegment) {
  ...
  buildGeneratedCustomerCode(branchId = null, sequence = 1) {
    const branchPrefix = this.getCustomerCodeBranchPrefix(branchId);
    const numericSequence = Number(sequence);
    const safeSequence = Number.isFinite(numericSequence) && numericSequence > 0
      ? Math.floor(numericSequence)
      : 1;
    return buildCustomerCodeFromPrefix(branchPrefix, safeSequence, GENERATED_CUSTOMER_CODE_WIDTH);
  }

  getNextGeneratedCustomerCodeSequence(branchId = null) {
    const branchPrefix = this.getCustomerCodeBranchPrefix(branchId);
    ...
    const row = this.db.prepare(
      ...
      `
    ).get(prefix.length + 1, GENERATED_CUSTOMER_CODE_WIDTH, codePattern);

    const maxSequence = Number(row?.max_sequence || 0);
    return Number.isFinite(maxSequence) && maxSequence > 0 ? maxSequence + 1 : 1;
  }

  generateUniqueCustomerCode(branchId = null) {
    const startingSequence = this.getNextGeneratedCustomerCodeSequence(branchId);
    for (let attempt = 0; attempt < 1000; attempt += 1) {
      const candidateCode = this.buildGeneratedCustomerCode(branchId, startingSequence + attempt);
      const exists = this.db.prepare(
        'SELECT id FROM customers WHERE UPPER(TRIM(customer_code)) = ? LIMIT 1'
      ).get(candidateCode);
  ...
  isCustomerCodeCompatibleWithBranch(customerCode, branchId = null) {
    const normalizedBranchId = this.normalizeCustomerRegistryBranchId(branchId);
    const normalizedCode = this.normalizeCustomerRegistryCode(customerCode);
    if (!normalizedBranchId || !normalizedCode) {
      return true;
    }

    const parsedCode = this.parseSequentialGeneratedCustomerCode(normalizedCode);
    if (!parsedCode) {
      return true;
    }

    return parsedCode.branchPrefix === this.getCustomerCodeBranchPrefix(normalizedBranchId);
  }

  isCustomerCodeAvailableForCustomer(customerCode, customerId = null) {
    const normalizedCode = this.normalizeCustomerRegistryCode(customerCode);
    if (!normalizedCode) {
      return true;
    }
    ...
    const row = this.db.prepare(
      `SELECT id
       ...
       WHERE UPPER(TRIM(COALESCE(customer_code, ''))) = ?
         AND (? <= 0 OR id <> ?)
       LIMIT 1`
    ).get(normalizedCode, numericCustomerId, numericCustomerId);

    return !row;
  }

  resolveSafeCustomerCodeForBranch(customerCode, branchId = null, customerId = null) {
    const normalizedCode = this.normalizeCustomerRegistryCode(customerCode);
    const normalizedBranchId = this.normalizeCustomerRegistryBranchId(branchId);
    if (
      normalizedCode
      && this.isCustomerCodeCompatibleWithBranch(normalizedCode, normalizedBranchId)
      && this.isCustomerCodeAvailableForCustomer(normalizedCode, customerId)
    ) {
      return normalizedCode;
    }

    return this.generateUniqueCustomerCode(normalizedBranchId);
  }

  updateCustomerCodeAcrossTransactionTables(customerId, oldCode, newCode, options = {}) {
    const numericCustomerId = Number(customerId || 0);
    const normalizedOldCode = this.normalizeCustomerRegistryCode(oldCode);
    const normalizedNewCode = this.normalizeCustomerRegistryCode(newCode);
    const normalizedBranchId = this.normalizeCustomerRegistryBranchId(options.branchId);
    const normalizedName = this.normalizeCustomerRegistryName(options.customerName);

    if (numericCustomerId <= 0 || !normalizedNewCode) {
      return { changes: 0 };
    }

    let changes = 0;
    const nextName = normalizedName || null;

    const updateByCustomerId = (tableName) => {
      const result = this.db.prepare(
        `UPDATE ${tableName}
         SET customer_code = ?,
             customer_name = COALESCE(?, customer_name)
         WHERE customer_id = ?`
      ).run(normalizedNewCode, nextName, numericCustomerId);
      changes += Number(result?.changes || 0);
    };

    const updateReconciledByScopedCode = (tableName) => {
      if (!normalizedOldCode || !normalizedBranchId || !normalizedName) {
        return;
      }
      ...
      const result = this.db.prepare(
        `UPDATE ${tableName}
         ...
         WHERE id IN (
           SELECT tx.id
           FROM ${tableName} tx
           LEFT JOIN reconciliations r ON r.id = tx.reconciliation_id
           LEFT JOIN cashiers c ON c.id = r.cashier_id
           WHERE COALESCE(tx.customer_id, 0) = 0
             AND UPPER(TRIM(COALESCE(tx.customer_code, ''))) = ?
             AND TRIM(COALESCE(tx.customer_name, '')) = ?
    ...
    const updateManualByScopedCode = (tableName) => {
      if (!options.allowManualNameCodeMatch || !normalizedOldCode || !normalizedName) {
        return;
      }
      ...
      ).run(numericCustomerId, normalizedNewCode, normalizedName, normalizedOldCode, normalizedName);
      changes += Number(result?.changes || 0);
    };
    ...
    [
      ...
      'manual_postpaid_sales',
      'manual_customer_receipts'
    ].forEach(updateByCustomerId);

    ['postpaid_sales', 'customer_receipts'].forEach(updateReconciledByScopedCode);
    ['manual_postpaid_sales', 'manual_customer_receipts'].forEach(updateManualByScopedCode);

    return { changes };
  }

  repairDuplicateCustomerCodes() {
    const repairDuplicates = this.db.transaction(() => {
      const duplicateCodes = this.db.prepare(
        `SELECT UPPER(TRIM(customer_code)) AS customer_code
         FROM customers
         WHERE TRIM(COALESCE(customer_code, '')) <> ''
         GROUP BY UPPER(TRIM(customer_code))
         HAVING COUNT(*) > 1
         ORDER BY UPPER(TRIM(customer_code)) ASC`
      ).all();
      ...
      const rowsByCodeStmt = this.db.prepare(
      ...
      duplicateCodes.forEach((duplicate) => {
        const normalizedDuplicateCode = this.normalizeCustomerRegistryCode(duplicate.customer_code);
        if (!normalizedDuplicateCode) {
          return;
        }

        const rows = rowsByCodeStmt.all(normalizedDuplicateCode);
        if (!Array.isArray(rows) || rows.length <= 1) {
          return;
        }

        const keeper = rows.find((row) => this.isCustomerCodeCompatibleWithBranch(
          row.customer_code,
          row.branch_id
        )) || rows[0];
        ...
        rows.forEach((row) => {
          ...
          const branchId = this.normalizeCustomerRegistryBranchId(row.branch_id);
          const nextCode = this.generateUniqueCustomerCode(branchId);
          updateCustomerStmt.run(nextCode, row.id);
          this.updateCustomerCodeAcrossTransactionTables(row.id, normalizedDuplicateCode, nextCode, {
            branchId,
    ...
    const result = repairDuplicates();
    if (result.repaired > 0) {
      console.log(`✅ [DB] تم إصلاح ${result.repaired} كود عميل مكرر`);
    }
    return result;
  }

  relinkCustomerTransactionsByUniqueCodes() {
    const relinkTransactions = this.db.transaction(() => {
      const customers = this.db.prepare(
        `SELECT id,
                TRIM(COALESCE(customer_name, '')) AS customer_name,
                UPPER(TRIM(COALESCE(customer_code, ''))) AS customer_code,
                COALESCE(branch_id, 0) AS branch_id
         FROM customers
         WHERE TRIM(COALESCE(customer_code, '')) <> ''
           AND UPPER(TRIM(COALESCE(customer_code, ''))) IN (
             SELECT customer_code
             FROM (
               SELECT UPPER(TRIM(COALESCE(customer_code, ''))) AS customer_code,
                      COUNT(*) AS code_count
               FROM customers
               WHERE TRIM(COALESCE(customer_code, '')) <> ''
               GROUP BY UPPER(TRIM(COALESCE(customer_code, '')))
             ) unique_codes
             WHERE code_count = 1
           )
         ORDER BY id ASC`
      ...
      if (!Array.isArray(customers) || customers.length === 0) {
        return { relinked: 0, normalized: 0 };
      }
      ...
      const reconciledTables = ['postpaid_sales', 'customer_receipts'];
      const manualTables = ['manual_postpaid_sales', 'manual_customer_receipts'];

      const normalizeReconciledByIdStatements = reconciledTables.map((tableName) => this.db.prepare(
        `UPDATE ${tableName}
         SET customer_name = ?,
             customer_code = ?
         WHERE customer_id = ?
           AND id IN (
             ...
             WHERE tx.customer_id = ?
               AND (
                 ? = 0
                 OR COALESCE(c.branch_id, 0) = 0
                 OR COALESCE(c.branch_id, 0) = ?
               )
           )`
      ));

      const normalizeManualByIdStatements = manualTables.map((tableName) => this.db.prepare(
        `UPDATE ${tableName}
         SET customer_name = ?,
             customer_code = ?
         WHERE customer_id = ?`
      ...
      const relinkReconciledByCodeStatements = reconciledTables.map((tableName) => this.db.prepare(
        `UPDATE ${tableName}
         SET customer_id = ?,
             customer_name = ?,
             customer_code = ?
         WHERE id IN (
           ...
           LEFT JOIN cashiers c ON c.id = r.cashier_id
           WHERE COALESCE(tx.customer_id, 0) = 0
             AND ${normalizedCodeSql('tx.customer_code')} = ?
             AND TRIM(COALESCE(tx.customer_name, '')) = ?
             AND (
               ? = 0
               OR COALESCE(c.branch_id, 0) = 0
      ...
      const relinkManualByCodeStatements = manualTables.map((tableName) => this.db.prepare(
        `UPDATE ${tableName}
         SET customer_id = ?,
             customer_name = ?,
             customer_code = ?
         WHERE COALESCE(customer_id, 0) = 0
           AND ${normalizedCodeSql('customer_code')} = ?
           AND TRIM(COALESCE(customer_name, '')) = ?`
      ));
      ...
      customers.forEach((customer) => {
        ...
        const customerName = this.normalizeCustomerRegistryName(customer.customer_name);
        const customerCode = this.normalizeCustomerRegistryCode(customer.customer_code);
        const customerBranchId = this.normalizeCustomerRegistryBranchId(customer.branch_id) || 0;
        if (!customerId || !customerName || !customerCode) {
          return;
        }

        normalizeReconciledByIdStatements.forEach((stmt) => {
          ...
          normalized += Number(result?.changes || 0);
        });

        normalizeManualByIdStatements.forEach((stmt) => {
          const result = stmt.run(customerName, customerCode, customerId);
          normalized += Number(result?.changes || 0);
        });

        relinkReconciledByCodeStatements.forEach((stmt) => {
          const result = stmt.run(
            customerId,
            customerName,
            customerCode,
          ...
          relinked += Number(result?.changes || 0);
        });

        relinkManualByCodeStatements.forEach((stmt) => {
          const result = stmt.run(customerId, customerName, customerCode, customerCode, customerName);
          relinked += Number(result?.changes || 0);
        });
      });
    ...
    const result = relinkTransactions();
    if (result.relinked > 0 || result.normalized > 0) {
      console.log(`✅ [DB] تم ربط ${result.relinked} حركة عميل قديمة وتوحيد ${result.normalized} حركة حسب أكواد العملاء الفريدة`);
    }
    return result;
  }

  repairCustomerTransactionBranchMismatches() {
    const repairMismatches = this.db.transaction(() => {
      const reconciledTables = ['postpaid_sales', 'customer_receipts'];
      let relinked = 0;
      let cleared = 0;

      reconciledTables.forEach((tableName) => {
        const relinkByBranchNameResult = this.db.prepare(
          `UPDATE ${tableName}
           SET customer_id = (
                 SELECT target.id
                 FROM customers target
                 LEFT JOIN reconciliations target_r ON target_r.id = ${tableName}.reconciliation_id
                 LEFT JOIN cashiers target_c ON target_c.id = target_r.cashier_id
                 WHERE COALESCE(target.branch_id, 0) = COALESCE(target_c.branch_id, 0)
                   AND TRIM(COALESCE(target.customer_name, '')) = TRIM(COALESCE(${tableName}.customer_name, ''))
                 ORDER BY target.id ASC
                 LIMIT 1
               ),
               customer_code = (
                 SELECT target.customer_code
                 FROM customers target
                 LEFT JOIN reconciliations target_r ON target_r.id = ${tableName}.reconciliation_id
                 LEFT JOIN cashiers target_c ON target_c.id = target_r.cashier_id
                 WHERE COALESCE(target.branch_id, 0) = COALESCE(target_c.branch_id, 0)
           ...
           WHERE id IN (
             SELECT tx.id
             FROM ${tableName} tx
             INNER JOIN customers current_customer ON current_customer.id = tx.customer_id
             LEFT JOIN reconciliations r ON r.id = tx.reconciliation_id
             LEFT JOIN cashiers c ON c.id = r.cashier_id
             WHERE COALESCE(tx.customer_id, 0) > 0
               AND COALESCE(c.branch_id, 0) > 0
               AND COALESCE(current_customer.branch_id, 0) > 0
               AND COALESCE(current_customer.branch_id, 0) <> COALESCE(c.branch_id, 0)
               AND 1 = (
                 SELECT COUNT(*)
                 FROM customers target
                 WHERE COALESCE(target.branch_id, 0) = COALESCE(c.branch_id, 0)
                   AND TRIM(COALESCE(target.customer_name, '')) = TRIM(COALESCE(tx.customer_name, ''))
               )
           )`
        ).run();
        relinked += Number(relinkByBranchNameResult?.changes || 0);

        const clearRemainingResult = this.db.prepare(
          `UPDATE ${tableName}
        ...
        cleared += Number(clearRemainingResult?.changes || 0);
      });

      return { relinked, cleared };
    });

    const result = repairMismatches();
    if (result.relinked > 0 || result.cleared > 0) {
      console.log(`✅ [DB] تم إصلاح ${result.relinked} حركة عميل بفرعها الصحيح ومسح ${result.cleared} ربط عميل متعارض مع الفرع`);
    }
    return result;
  }

  normalizeGeneratedCustomerCodes() {
    const normalizeLegacyCodes = this.db.transaction(() => {
      const legacyCustomers = this.db.prepare(
        `
          SELECT id, customer_code, customer_name, branch_id
          FROM customers
          WHERE TRIM(COALESCE(customer_code, '')) <> ''
          ORDER BY id ASC
        `
      ).all().filter((row) => this.isLegacyGeneratedCustomerCode(row.customer_code));

      if (legacyCustomers.length === 0) {
        return 0;
      }

      legacyCustomers.forEach((customerRow) => {
        const normalizedOldCode = this.normalizeCustomerRegistryCode(customerRow.customer_code);
        const rowBranchId = this.normalizeCustomerRegistryBranchId(customerRow.branch_id);
        const codeBranchId = this.parseGeneratedCustomerCodeBranchId(normalizedOldCode);
        const nextCode = this.generateUniqueCustomerCode(rowBranchId || codeBranchId || null);
        ...
        this.db.prepare(
          `UPDATE customers
           SET customer_code = ?,
               updated_at = CURRENT_TIMESTAMP
           WHERE id = ?`
        ).run(nextCode, customerRow.id);

        this.updateCustomerCodeAcrossTransactionTables(customerRow.id, normalizedOldCode, nextCode, {
          branchId: rowBranchId,
          customerName: customerRow.customer_name,
          allowManualNameCodeMatch: true
        });
      });
    ...
    const migratedCount = normalizeLegacyCodes();
    if (migratedCount > 0) {
      console.log(`✅ [DB] تم تحويل ${migratedCount} كود عميل قديم إلى الصيغة التسلسلية الجديدة`);
    }
  }

  normalizeCustomerCodeBranchSegments() {
    const normalizeBranchSegments = this.db.transaction(() => {
      ...
      customerRows.forEach((customerRow) => {
        const parsedCode = this.parseSequentialGeneratedCustomerCode(customerRow.customer_code);
        const branchId = this.normalizeCustomerRegistryBranchId(customerRow.branch_id);
        if (!parsedCode || !branchId) {
          return;
        }

        const targetBranchPrefix = this.getCustomerCodeBranchPrefix(branchId);
        if (targetBranchPrefix === parsedCode.branchPrefix) {
          return;
        }

        const oldCode = this.normalizeCustomerRegistryCode(customerRow.customer_code);
        let targetCode = buildCustomerCodeFromPrefix(
          targetBranchPrefix,
          parsedCode.sequence,
          GENERATED_CUSTOMER_CODE_WIDTH
        );
        const conflict = conflictStmt.get(targetCode, customerRow.id);
        if (conflict) {
          targetCode = this.generateUniqueCustomerCode(branchId);
        }

        updateStmt.run(targetCode, customerRow.id);
        this.updateCustomerCodeAcrossTransactionTables(customerRow.id, oldCode, targetCode, {
          branchId,
          customerName: customerRow.customer_name,
          allowManualNameCodeMatch: true
        });
    ...
    const result = normalizeBranchSegments();
    if (result.migrated > 0) {
      console.log(`✅ [DB] تم تعديل ${result.migrated} كود عميل ليتوافق مع رموز الفروع الثابتة`);
    }
    if (result.skipped > 0) {
      console.warn(`⚠️ [DB] تم تجاوز ${result.skipped} كود عميل بسبب تعارض في الكود الجديد`);
    }
    return result;
  }

  ensureCustomerRegistryRecord({ customerName, customerCode = '', branchId = null, phone = '', address = '' }) {
    const normalizedName = this.normalizeCustomerRegistryName(customerName);
    if (!normalizedName) {
      return null;
    }

    const normalizedBranchId = this.normalizeCustomerRegistryBranchId(branchId);
    let normalizedCode = this.normalizeCustomerRegistryCode(customerCode);

    let customerRow = null;
    if (normalizedCode) {
      const codeRows = this.db.prepare(
        `SELECT *
         FROM customers
         WHERE UPPER(TRIM(COALESCE(customer_code, ''))) = ?
         ORDER BY id ASC`
      ).all(normalizedCode);
      customerRow = (Array.isArray(codeRows) ? codeRows : []).find((row) => {
        const rowName = this.normalizeCustomerRegistryName(row.customer_name);
        const rowBranchId = this.normalizeCustomerRegistryBranchId(row.branch_id);
        const sameName = rowName === normalizedName;
        const sameBranch = !normalizedBranchId || !rowBranchId || rowBranchId === normalizedBranchId;
        return sameName && sameBranch;
      }) || null;

      if (!customerRow && Array.isArray(codeRows) && codeRows.length > 0) {
        normalizedCode = '';
      }
    }
    ...
    if (!customerRow && normalizedBranchId) {
      customerRow = this.db.prepare(
        `SELECT *
         FROM customers
         ...
         ORDER BY id ASC
         LIMIT 1`
      ).get(normalizedName, normalizedBranchId);
    }

    if (!customerRow && !normalizedCode) {
      customerRow = this.db.prepare(
        `SELECT *
         FROM customers
         WHERE TRIM(customer_name) = ?
    ...
    if (customerRow) {
      const nextBranchId = normalizedBranchId || this.normalizeCustomerRegistryBranchId(customerRow.branch_id);
      const existingCode = this.normalizeCustomerRegistryCode(customerRow.customer_code);
      const nextCode = this.resolveSafeCustomerCodeForBranch(
    ...
    normalizedCode = this.resolveSafeCustomerCodeForBranch(normalizedCode, normalizedBranchId);
    const insertResult = this.db.prepare(
      `INSERT INTO customers (
         customer_code, customer_name, branch_id, phone, address, created_at, updated_at
  ...
  syncCustomerRegistryFromTransactions() {
    const customerRows = this.db.prepare(`
      SELECT DISTINCT customer_name, customer_code, branch_id
      FROM (
        SELECT
          ps.customer_name AS customer_name,
          COALESCE(ps.customer_code, '') AS customer_code,
        ...
        SELECT
          ...
          COALESCE(cr.customer_code, '') AS customer_code,
          COALESCE(c.branch_id, 0) AS branch_id
        ...
        SELECT
          mps.customer_name AS customer_name,
          COALESCE(mps.customer_code, '') AS customer_code,
          COALESCE((SELECT branch_id FROM cashiers ORDER BY id LIMIT 1), 0) AS branch_id
        FROM manual_postpaid_sales mps

        UNION ALL

        SELECT
          mcr.customer_name AS customer_name,
          COALESCE(mcr.customer_code, '') AS customer_code,
          COALESCE((SELECT branch_id FROM cashiers ORDER BY id LIMIT 1), 0) AS branch_id
    ...
    const updateReconciledByNameAndBranch = (tableName, customer) => {
      const normalizedBranchId = this.normalizeCustomerRegistryBranchId(customer?.branch_id) || 0;
      this.db.prepare(
        `UPDATE ${tableName}
         SET customer_id = ?,
             customer_code = ?,
         ...
         WHERE id IN (
           ...
           LEFT JOIN reconciliations r ON r.id = tx.reconciliation_id
           LEFT JOIN cashiers c ON c.id = r.cashier_id
           WHERE TRIM(COALESCE(tx.customer_name, '')) = ?
             AND UPPER(TRIM(COALESCE(tx.customer_code, ''))) IN ('', '-', '–', '—')
             AND COALESCE(c.branch_id, 0) = ?
         )`
      ).run(customer.id, customer.customer_code, customer.customer_name, customer.customer_name, normalizedBranchId);
    };

    const updateManualByName = (tableName, customer) => {
      this.db.prepare(
        `UPDATE ${tableName}
         SET customer_id = ?,
             customer_code = ?,
         ...
         WHERE TRIM(COALESCE(customer_name, '')) = ?
           AND UPPER(TRIM(COALESCE(customer_code, ''))) IN ('', '-', '–', '—')`
      ).run(customer.id, customer.customer_code, customer.customer_name, customer.customer_name);
    };

    customerRows.forEach((row) => {
      const customer = this.ensureCustomerRegistryRecord({
        customerName: row.customer_name,
      ...
      const sourceCode = this.normalizeCustomerRegistryCode(row.customer_code);
      if (customer.customer_code && sourceCode) {
        this.updateCustomerCodeAcrossTransactionTables(customer.id, sourceCode, customer.customer_code, {
          branchId: row.branch_id,
          customerName: row.customer_name,
          allowManualNameCodeMatch: true
        });
      ...
      updateReconciledByNameAndBranch('postpaid_sales', customer);
      updateReconciledByNameAndBranch('customer_receipts', customer);
      updateManualByName('manual_postpaid_sales', customer);
      updateManualByName('manual_customer_receipts', customer);
    });
  }
    ...
    try {
      ...
      try {
        reconciliation = this.get(`
          SELECT r.*, c.name as cashier_name, c.cashier_number, c.branch_id as cashier_branch_id, a.name as accountant_name, b.branch_name
          FROM reconciliations r
      ...
      let suppliers = [];
      let customTables = [];
      const reconciliationBranchId = this.normalizeCustomerRegistryBranchId(reconciliation.cashier_branch_id);
      const customerCodeExpression = (alias) => `
        NULLIF(NULLIF(NULLIF(NULLIF(UPPER(TRIM(COALESCE(${alias}.customer_code, ''))), ''), '-'), '–'), '—')
      `;
      const customerNameCodeLookup = (alias) => `
        SELECT ${customerCodeExpression('cust')}
        FROM customers cust
        WHERE TRIM(COALESCE(cust.customer_name, '')) = TRIM(COALESCE(${alias}.customer_name, ''))
          AND COALESCE(cust.branch_id, 0) = COALESCE(?, 0)
        ORDER BY cust.id ASC
        LIMIT 1
      `;
      const customerNameIdLookup = (alias) => `
        SELECT cust.id
        FROM customers cust
        WHERE TRIM(COALESCE(cust.customer_name, '')) = TRIM(COALESCE(${alias}.customer_name, ''))
          AND COALESCE(cust.branch_id, 0) = COALESCE(?, 0)
      ...
      try {
        ...
        postpaidSales = this.query(`
          SELECT
            ps.id,
            ps.reconciliation_id,
            COALESCE(ps.customer_id, (${customerNameIdLookup('ps')})) AS customer_id,
            ps.customer_name,
            COALESCE(
              ${customerCodeExpression('ps')},
              (SELECT ${customerCodeExpression('cust')} FROM customers cust WHERE cust.id = ps.customer_id LIMIT 1),
              (${customerNameCodeLookup('ps')}),
              ''
            ) AS customer_code,
            ps.amount,
            ps.is_modified,
            ps.created_at
          FROM postpaid_sales ps
          WHERE ps.reconciliation_id = ?
      ORDER BY created_at
      `, [reconciliationBranchId, reconciliationBranchId, numericId]) || [];
      ...
      try {
        ...
        customerReceipts = this.query(`
          SELECT
            cr.id,
            cr.reconciliation_id,
            COALESCE(cr.customer_id, (${customerNameIdLookup('cr')})) AS customer_id,
            cr.customer_name,
            COALESCE(
              ${customerCodeExpression('cr')},
              (SELECT ${customerCodeExpression('cust')} FROM customers cust WHERE cust.id = cr.customer_id LIMIT 1),
              (${customerNameCodeLookup('cr')}),
              ''
            ) AS customer_code,
            cr.amount,
            cr.payment_type,
