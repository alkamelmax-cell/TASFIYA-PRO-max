...
class ThermalPrinter80mm {
    ...
    estimateThermalPageHeightMicrons(reconciliationData, htmlContent = '') {
        ...
        const defaultMaxHeightMm = 2000;
        const longStructuredReportMaxHeightMm = 6000;
        const fontSize = Number(this.settings.fontSize) || 9;
        const lineHeightMm = Math.max(4.5, Math.min(7, fontSize * 0.68));

        const clampHeightMm = (value, maxHeightMm = defaultMaxHeightMm) => (
            Math.min(Math.max(value, minHeightMm), maxHeightMm)
        );
            ...
            try {
                ...
                if (parsedStatement?.isStructuredStatement && parsedStatement.statementType === 'postpaid_net_balances') {
                    ...
                    for (const row of rows) {
                        ...
                        const wrappedLines = Math.max(1, Math.ceil(customerName.length / 18));
                        rowHeightMm += 6.5 + ((wrappedLines - 1) * 3.5);
                    }

                    const headerMm = 48;
                    const summaryMm = parsedStatement.summary ? 22 : 0;
                    const footerMm = 18;
                    return Math.round(
                        clampHeightMm(
                            headerMm + rowHeightMm + summaryMm + footerMm,
                            longStructuredReportMaxHeightMm
                        ) * 1000
                    );
                }
    ...
    generateBankReceiptsHTML(bankReceipts, total) {
        const hasPerOperationDateTime = bankReceipts.some((item) =>
            Boolean(item?.thermal_operation_datetime || item?.operation_datetime || item?.formatted_datetime)
        );

        if (hasPerOperationDateTime) {
            const rows = bankReceipts.map((item, index) => {
                const operationDateTime = item.thermal_operation_datetime
                    || (item.formatted_datetime ? String(item.formatted_datetime).trim().replace(/\s+/, '<br>') : 'غير محدد');
                const operationDetails = item.operation_type
                    ? `${item.operation_type}${item.bank_name ? ` - ${item.bank_name}` : ''}`
                    : (item.bank_name || 'بدون بنك');

                return `
            <tr>
                <td style="font-size: 12px; text-align: center;">${index + 1}</td>
                <td style="font-size: 10px; text-align: center; line-height: 1.2; white-space: nowrap;">${operationDateTime}</td>
                <td style="font-size: 12px; line-height: 1.2;">
                    <div style="font-weight: 700;">${item.atm_name || 'بدون جهاز'}</div>
                    <div style="font-size: 10px; margin-top: 2px;">${operationDetails}</div>
                </td>
                <td style="font-size: 12px; text-align: right; white-space: nowrap;">${(item.amount || 0).toFixed(2)}</td>
            </tr>
        `;
            }).join('');
                ...
                <table style="width: 100%; border-collapse: collapse; font-size: 12px; table-layout: fixed;">
                    <thead>
                        <tr style="background: #f0f0f0;">
                            <th style="border: 1px solid #666; padding: 3px; text-align: center; width: 10%;">الرقم</th>
                            <th style="border: 1px solid #666; padding: 3px; text-align: center; width: 22%;">التاريخ والوقت</th>
                            <th style="border: 1px solid #666; padding: 3px; text-align: right; width: 44%;">البيان</th>
                            <th style="border: 1px solid #666; padding: 3px; text-align: right; width: 24%;">المبلغ</th>
                        </tr>
                    </thead>
                    <tbody>
                        ${rows}
    ...
    generatePostpaidNetBalancesStatementHTML(stmtData, reconciliationData) {
        ...
        const normalizeRowForPrint = (row) => {
        ...
        const printableRows = tableData.map(normalizeRowForPrint);

        const estimateRowHeightMm = (row) => {
            const customerName = String(row?.customerName || '').trim();
            const customerCode = String(row?.customerCode || '').trim();
            const combinedLength = customerName.length + (customerCode ? (customerCode.length + 3) : 0);
            const wrappedLines = Math.max(1, Math.ceil(combinedLength / 18));
            return 6.5 + ((wrappedLines - 1) * 3.5);
        };

        const regularPageBudgetMm = 210;
        const finalPageBudgetMm = 182;
        const estimatePageHeight = (rows) => rows.reduce((sum, row) => sum + estimateRowHeightMm(row), 0);

        const splitRowsByBudget = (rows, budgetMm) => {
            if (!rows.length) {
                return [[]];
            }
            ...
            rows.forEach((row) => {
                ...
                if (currentPage.length > 0 && (currentHeight + rowHeight) > budgetMm) {
        ...
        const paginateRows = (rows) => {
            const pages = splitRowsByBudget(rows, regularPageBudgetMm);

            while (pages.length > 0) {
                const lastPage = pages[pages.length - 1];
                if (estimatePageHeight(lastPage) <= finalPageBudgetMm || lastPage.length <= 1) {
                    break;
                }

                const rowsToReflow = pages.pop();
                let splitIndex = 0;
                let currentHeight = 0;

                for (let index = 0; index < rowsToReflow.length; index += 1) {
                    const rowHeight = estimateRowHeightMm(rowsToReflow[index]);
                    if (splitIndex > 0 && (currentHeight + rowHeight) > regularPageBudgetMm) {
                        break;
                    }
                ...
                if (splitIndex >= rowsToReflow.length) {
                    splitIndex = Math.max(1, rowsToReflow.length - 1);
                }

                pages.push(rowsToReflow.slice(0, splitIndex));
                pages.push(rowsToReflow.slice(splitIndex));
            }

            return pages.filter((pageRows) => pageRows.length > 0 || rows.length === 0);
        };

        const paginatedRows = paginateRows(printableRows);
        const usesFixedThermalPages = paginatedRows.length > 1;
        const pageSizeCss = usesFixedThermalPages ? '72mm 297mm' : '72mm auto';
        ...
        const footerHtml = `
            <div class="footer">
                <div>━━━━━━━━━━━━━━━━━━━━━━━━━━</div>
                <div><strong>تصفية برو</strong> | النسخة 5.0.2</div>
        ...
        const renderHeaderHtml = (pageNumber, pageCount) => `
            <div class="header">
                ...
                <div class="header-meta"><strong>عدد العملاء:</strong> ${countLabel}${pageCount > 1 ? ` | <strong>الصفحة:</strong> ${pageNumber} / ${pageCount}` : ''}</div>
                ${filterLine}
            </div>
        `;
        ...
        const renderTableHtml = (rows) => {
            const rowHtml = rows.map((row) => {
                ...
                return `
                    <tr>
                        <td class="cell index-cell">${renderedRowNumber}</td>
                        <td class="cell customer-cell">
                            <div class="customer-name-line">${escapeHtml(row.customerName)}</div>
                            ${row.customerCode ? `<div class="customer-code-line">#${escapeHtml(row.customerCode)}</div>` : ''}
                        </td>
                        <td class="cell amount-cell debit-cell">${fmt(row.debitValue)}</td>
                        <td class="cell amount-cell credit-cell">${fmt(row.creditValue)}</td>
                    </tr>
                `;
            }).join('');
        ...
        const pagesHtml = paginatedRows.map((pageRows, pageIndex) => {
            const pageNumber = pageIndex + 1;
            const pageCount = paginatedRows.length;
            const isLastPage = pageIndex === (pageCount - 1);

            return `
                <section class="receipt-page${isLastPage ? ' last-page' : ''}">
                    ${renderHeaderHtml(pageNumber, pageCount)}
                    ${renderTableHtml(pageRows)}
                    ${isLastPage ? summaryHtml : `<div class="continuation-note">يتبع في الصفحة التالية</div>`}
            ...
            <style>
                * { margin: 0; padding: 0; box-sizing: border-box; }
                ...
                .customer-code-line {
                    display: block;
                    margin-top: 1px;
                    font-size: ${compactFontSize - 0.35}pt;
                    font-weight: 700;
                    color: #333;
                    direction: ltr;
                    text-align: right;
                ...
                @media print {
                    ...
                    .summary-section, .footer { page-break-inside: avoid; }
                    .receipt-page {
                        page-break-after: always;
                        break-after: page;
                    }
        ...
        <body data-thermal-page-mode="${usesFixedThermalPages ? 'fixed' : 'auto'}">
            ${pagesHtml}
        </body>
        </html>
        `;
    ...
    generateCustomStatementHTML(textContent, reconciliationData) {
        ...
        const dateRangeLabel = String(stmtData.statementDateRange || '').trim();
        ...
        <body>
            <div class="header">
                ...
                ${dateRangeLabel ? `<div class="header-date"><strong>الفترة:</strong> ${dateRangeLabel}</div>` : ''}
            </div>
            ...
            <div class="summary-section">
                ...
                <div class="summary-row">
                    <span class="summary-label">رصيد مرحل:</span>
                    <span class="summary-value">${openingBalanceStr}</span>
                </div>` : ''}
                <div class="summary-row postpaid-row">
    ...
    async printReceipt(reconciliationData, printerName = null) {
        ...
        try {
            ...
            const usesFixedThermalPageMode = htmlContent.includes('data-thermal-page-mode="fixed"');
            const effectivePageHeightMicrons = usesFixedThermalPageMode ? 297000 : estimatedPageHeightMicrons;
            ...
            const printOptions = {
                ...
                pageSize: {
                    ...
                    height: effectivePageHeightMicrons  // ارتفاع الصفحة الحرارية الفعلي
                },
            ...
            const postPrintDelayMs = Math.min(
                8000,
                Math.max(1500, Math.ceil(estimatedPageHeightMicrons / 1000) * 4)
            );
