...
function normalizeCustomerRecord(row) {
    if (typeof row === 'string') {
        const name = row.trim();
        return name ? { customer_name: name, customer_code: '', customer_id: null, branch_id: null } : null;
    }
    ...
    const customerName = String(row.customer_name || row.name || '').trim();
    if (!customerName) {
        return null;
    }
    ...
    return {
        ...
        customer_code: normalizeCustomerCode(row.customer_code || row.code),
...
function getCustomerDisplayValue(customer) {
    ...
    return record.customer_code
        ? `${record.customer_name} - ${record.customer_code}`
        : record.customer_name;
}

function formatCustomerLineLabel(item) {
    const name = String(item && item.customer_name ? item.customer_name : '').trim();
    const code = normalizeCustomerCode(item && item.customer_code ? item.customer_code : '');
    return code ? `${name} - ${code}` : name;
}

function normalizeCustomerLineItem(item) {
    const normalized = normalizeCustomerRecord(item);
    if (!normalized) {
        return {
            ...item,
            customer_display: formatCustomerLineLabel(item)
        };
    }
    ...
    return {
        ...
        customer_display: getCustomerDisplayValue(normalized)
    };
}

function resolveCustomerInput(inputValue) {
    const normalizedInput = String(inputValue || '').trim();
    if (!normalizedInput) {
        return null;
    }

    const exactDisplay = customerRecords.find((record) => getCustomerDisplayValue(record) === normalizedInput);
    if (exactDisplay) {
        return exactDisplay;
    }

    const exactNameMatches = customerRecords.filter((record) => record.customer_name === normalizedInput);
    if (exactNameMatches.length === 1) {
        return exactNameMatches[0];
    }

    const displayMatch = normalizedInput.match(/^(.*?)\s+-\s+([A-Za-z0-9]+-\d{1,})$/);
    if (displayMatch && displayMatch[1].trim()) {
        return {
            customer_id: null,
            customer_name: displayMatch[1].trim(),
            customer_code: normalizeCustomerCode(displayMatch[2]),
            branch_id: null
        };
    }
...
function renderAllTables() {
    ...
    renderTable('postpaidTableBody', postpaidItems, ['customer_display', 'amount']);
    renderTable('custReceiptTableBody', custReceiptItems, ['customer_display', 'type', 'amount']);
...
async function restoreRequestDraft(options = {}) {
    try {
        ...
        postpaidItems = Array.isArray(lists.postpaidItems)
            ? lists.postpaidItems.map(normalizeCustomerLineItem)
            : [];
        custReceiptItems = Array.isArray(lists.custReceiptItems)
            ? lists.custReceiptItems.map(normalizeCustomerLineItem)
            : [];
...
async function loadCustomers() {
    ...
    const rawCustomers = Array.isArray(result.customer_records) && result.customer_records.length > 0
        ? result.customer_records
        : (result.customers || []);
    customerRecords = rawCustomers.map(normalizeCustomerRecord).filter(Boolean);
    list.innerHTML = customerRecords
        .map((customer) => {
            const value = getCustomerDisplayValue(customer);
            const label = customer.customer_code || customer.customer_name;
            return `<option value="${escapeHtml(value)}" label="${escapeHtml(label)}">`;
        })
        .join('');
...
function addPostpaidItem() {
    const inputValue = document.getElementById('postpaidName').value.trim();
    const selectedCustomer = resolveCustomerInput(inputValue);
    const customer_name = selectedCustomer ? selectedCustomer.customer_name : inputValue;
    ...
    postpaidItems.push(normalizeCustomerLineItem({
        ...
        branch_id: selectedCustomer ? selectedCustomer.branch_id : null,
        amount: amt
    }));
    renderTable('postpaidTableBody', postpaidItems, ['customer_display', 'amount']);
...
function addCustomerReceiptItem() {
    const inputValue = document.getElementById('custReceiptName').value.trim();
    const selectedCustomer = resolveCustomerInput(inputValue);
    const customer_name = selectedCustomer ? selectedCustomer.customer_name : inputValue;
    ...
    custReceiptItems.push(normalizeCustomerLineItem({
        customer_id: selectedCustomer ? selectedCustomer.customer_id : null,
        customer_name,
        customer_code: selectedCustomer ? selectedCustomer.customer_code : '',
        branch_id: selectedCustomer ? selectedCustomer.branch_id : null,
