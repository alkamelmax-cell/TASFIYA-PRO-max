...
function registerIpcHandlers() {
    ...
    ipcMain.handle('client-sender:fetch-customers', async () => {
        const normalizeCustomerRecords = (customers) => (Array.isArray(customers) ? customers : [])
            .map((customer) => {
                ...
                const customerName = String(customer.customer_name || customer.name || '').trim();
                if (!customerName) {
                    return null;
                }

                const customerId = Number(customer.customer_id || customer.id || 0);
                const branchId = Number(customer.branch_id || 0);
                const customerCode = String(customer.customer_code || customer.code || '').trim().toUpperCase();
                return {
                    customer_id: Number.isFinite(customerId) && customerId > 0 ? customerId : null,
                    customer_name: customerName,
                    customer_code: ['-', '–', '—'].includes(customerCode) ? '' : customerCode,
                    branch_id: Number.isFinite(branchId) && branchId > 0 ? branchId : null
                };
            })
            .filter(Boolean);
        const customerNames = (records) => records.map((customer) => customer.customer_name).filter(Boolean);

        if (clientState.offlineMode && clientDb) {
            const cachedCustomerRecords = normalizeCustomerRecords(clientDb.listCachedCustomers());
            return createCacheSuccessResponse(
                'customers',
                customerNames(cachedCustomerRecords),
                'التطبيق يعمل الآن من النسخة المحلية.',
                { offline: true, customer_records: cachedCustomerRecords }
            );
        }
        ...
        try {
            ...
            const customerRecords = normalizeCustomerRecords(
                result.data && Array.isArray(result.data.customer_records) && result.data.customer_records.length > 0
                    ? result.data.customer_records
                    : (result.data && Array.isArray(result.data.customers) ? result.data.customers : [])
            );
            if (clientDb) {
                clientDb.cacheCustomers(customerRecords);
            }

            return createRemoteSuccessResponse('customers', customerNames(customerRecords), {
            ...
            if (!authError && clientDb) {
                const cachedCustomerRecords = normalizeCustomerRecords(clientDb.listCachedCustomers());
                if (cachedCustomerRecords.length > 0) {
                    return createCacheSuccessResponse(
                        'customers',
                        customerNames(cachedCustomerRecords),
                        error.message,
                        { offline: isNetworkFailure(error), customer_records: cachedCustomerRecords }
                    );
                }
            }

            if (authError && fallbackToOfflineSession()) {
                const cachedCustomerRecords = clientDb ? normalizeCustomerRecords(clientDb.listCachedCustomers()) : [];
                return createCacheSuccessResponse(
                    'customers',
                    customerNames(cachedCustomerRecords),
                    cachedCustomerRecords.length > 0
