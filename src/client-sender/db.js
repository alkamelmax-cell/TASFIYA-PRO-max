...
class ClientSenderDb {
    ...
    createTables() {
        this.db.exec(`
            ...
            CREATE TABLE IF NOT EXISTS cached_customer_records (
            ...
            CREATE INDEX IF NOT EXISTS idx_cached_customer_records_name
            ON cached_customer_records(name);

            CREATE INDEX IF NOT EXISTS idx_cached_customer_records_code
            ON cached_customer_records(customer_code);
    ...
    cacheCustomers(customers = []) {
        ...
        const normalizeCustomer = (customer) => {
            ...
            const remoteId = Number(customer.customer_id || customer.id || 0);
            ...
            const cacheKey = remoteId > 0
                ? `id:${remoteId}`
                : `${name}|${customerCode}|${Number.isFinite(branchId) && branchId > 0 ? branchId : ''}`;

            return {
                cache_key: cacheKey,
                remote_id: Number.isFinite(remoteId) && remoteId > 0 ? remoteId : null,
                name,
        ...
        const legacyInsertStmt = this.db.prepare(`
            ...
            ON CONFLICT(name) DO UPDATE SET
                ...
                updated_at = excluded.updated_at
        `);
        const recordInsertStmt = this.db.prepare(`
            INSERT INTO cached_customer_records (
                cache_key,
                remote_id,
                name,
                customer_code,
        ...
        const runTransaction = this.db.transaction((rows) => {
            this.db.prepare('DELETE FROM cached_customer_records').run();
            rows.forEach((customer) => {
                ...
                legacyInsertStmt.run(normalized.name, now, now);
                recordInsertStmt.run(
    ...
    listCachedCustomers() {
        const recordRows = this.db.prepare(`
            SELECT remote_id AS customer_id, name AS customer_name, customer_code, branch_id
            FROM cached_customer_records
            ORDER BY name COLLATE NOCASE ASC, customer_code ASC
        `).all();
