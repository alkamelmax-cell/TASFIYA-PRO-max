...
(function () {
    ...
    function createRequestsSection() {
        ...
        style.textContent = `
            ...
            body.theme-sender .req-header-wrapper {
                background: rgba(255, 251, 244, 0.92);
                border-color: rgba(33, 79, 67, 0.14);
                box-shadow: 0 12px 28px rgba(120, 91, 44, 0.12);
            }
            ...
            body.theme-sender .req-main-title {
                color: #17392f;
            }
            ...
            body.theme-sender .req-subtitle {
                color: #6b7d75;
            }
            ...
            body.theme-sender .req-tab-container {
                background: rgba(231, 181, 108, 0.12);
                border-color: rgba(33, 79, 67, 0.14);
            }
            ...
            body.theme-sender .req-tab-btn {
                color: #35584e;
            }

            body.theme-sender .req-tab-btn:hover {
                color: #17392f;
                background: rgba(231, 181, 108, 0.16);
            }
            ...
            body.theme-sender .req-tab-btn.active {
                color: #fffaf2;
                background: linear-gradient(135deg, #17392f 0%, #214f43 100%);
                box-shadow: 0 6px 14px rgba(23, 57, 47, 0.22);
            }
            ...
            body.theme-sender .req-refresh-btn {
                color: #fffaf2;
                background: linear-gradient(135deg, #d7a159 0%, #d48443 100%);
                box-shadow: 0 8px 18px rgba(212, 132, 67, 0.24);
            }

            body.theme-sender .req-refresh-btn:hover {
                box-shadow: 0 10px 22px rgba(212, 132, 67, 0.3);
            }
            ...
            body.theme-sender .req-card {
                background: rgba(255, 251, 244, 0.94);
                border-color: rgba(33, 79, 67, 0.12);
                box-shadow: 0 14px 34px rgba(120, 91, 44, 0.12);
            }
            ...
            body.theme-sender .req-table-header {
                background: linear-gradient(135deg, #17392f 0%, #214f43 100%);
                color: #fffaf2;
            }
            ...
            body.theme-sender .req-row {
                border-bottom-color: rgba(33, 79, 67, 0.08);
            }

            body.theme-sender .req-row:hover {
                background-color: rgba(231, 181, 108, 0.12);
            }
            ...
            body.theme-sender .req-date-main,
            body.theme-sender .req-cashier-name {
                color: #17392f;
            }
            ...
            body.theme-sender .req-date-sub,
            body.theme-sender .req-cashier-branch,
            body.theme-sender .req-reviewed-label {
                color: #6c7e76;
            }
            ...
            body.theme-sender .req-empty-state {
                color: #708078;
            }
            ...
            body.theme-sender .req-pagination {
                border-top-color: rgba(33, 79, 67, 0.12);
                background: rgba(231, 181, 108, 0.08);
            }
            ...
            body.theme-sender .req-page-btn {
                border-color: rgba(33, 79, 67, 0.14);
                background: rgba(255, 251, 244, 0.98);
                color: #214f43;
            }

            body.theme-sender .req-page-btn:hover:not(:disabled) {
                color: #fffaf2;
                border-color: transparent;
                background: linear-gradient(135deg, #d7a159 0%, #d48443 100%);
            }

            body.theme-sender .req-page-btn.active {
                color: #fffaf2;
                border-color: transparent;
                background: linear-gradient(135deg, #17392f 0%, #214f43 100%);
                box-shadow: 0 6px 14px rgba(23, 57, 47, 0.22);
            ...
            body.theme-sender .req-pagination-info {
                color: #6b7d75;
            }
    ...
    function buildRequestsWhereClause(status) {
        ...
        if (status === 'completed') {
            return {
                sql: "WHERE r.status IN ('completed', 'approved', 'مكتملة', 'معتمدة')",
                params: []
            };
        }
        ...
        if (status === 'pending') {
            return {
                sql: "WHERE (r.status IS NULL OR r.status = '' OR r.status IN ('pending', 'معلقة', 'قيد الانتظار'))",
                params: []
            };
        }
    ...
    async function loadRequestsFromServer(status, page, limit) {
        ...
        const response = await fetch(
            `http://localhost:4000/api/reconciliation-requests?${params.toString()}`
        );
    ...
    async function refreshRemoteRequestsInBackground(status, page, loadTokenAtStart) {
        if (!hasDesktopDbBridge()) {
            return;
        }

        try {
            const result = await requestsIpc.invoke('pull-reconciliation-requests');
            if (!result || !result.success) {
                if (!result || !result.skipped) {
                    console.warn('⚠️ [REQUESTS] Remote request pull did not complete successfully:', result);
                }
                return;
            }

            if (
                requestLoadToken !== loadTokenAtStart
            ...
            console.warn('⚠️ [REQUESTS] Failed to refresh reconciliation requests from remote:', error);
        }
    }

    async function loadRequests(status = currentFilter, page = 1, options = {}) {
        ...
        if (!loadOptions.preserveTableState) {
            tbody.innerHTML = '<tr><td colspan="7" class="text-center"><i class="icon">⏳</i> جاري التحميل...</td></tr>';
        }

        if (hasDesktopDbBridge() && !loadOptions.skipRemoteRefresh) {
            refreshRemoteRequestsInBackground(currentFilter, currentPage, loadToken);
        }
    ...
    async function fetchRequestDetailsFromServer(requestId) {
        const response = await fetch(
            `http://localhost:4000/api/reconciliation-requests/${requestId}?include_details=raw`
        );
    ...
    async function fetchRequestDetails(requestId) {
        ...
        if (hasDesktopDbBridge()) {
            try {
                return await fetchRequestDetailsFromLocalDb(requestId);
            } catch (error) {
                localError = error;
    ...
    async function openRequestAsReconciliation(requestId) {
        ...
        const pendingData = {
            ...
            total_bank: request.total_bank || 0,
            details: details,
    ...
    window.addEventListener('reconciliation-saved', (event) => {
        ...
        if (originRequestId) {
            markRequestAsDone(originRequestId).catch(error => {
                console.error('❌ [EVENT] Failed to mark request as done:', error);
            });
        } else {
    ...
    async function completeRequestLocally(requestId) {
        if (!hasDesktopDbBridge()) {
            return false;
        }
    ...
    async function markRequestAsDone(requestId) {
        ...
        try {
            const completedLocally = await completeRequestLocally(requestId);
            if (completedLocally) {
                console.log('✅ [LOCAL] Request marked as completed in local DB:', reqIdStr);
            } else {
                console.warn('⚠️ [LOCAL] Request completion did not update a local DB row:', reqIdStr);
            }
        } catch (error) {
            console.error('❌ [LOCAL] Failed to mark request as completed locally:', error);
        }
