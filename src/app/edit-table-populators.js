function createEditTablePopulators(deps) {
  ...
  function getCustomerCodeDisplay(record) {
    const code = String(record?.customer_code || '').trim().toUpperCase();
  ...
  function populateEditPostpaidSalesTable(postpaidSales) {
    ...
    postpaidSales.forEach((sale, index) => {
      const row = document.createElement('tr');
      row.innerHTML = `
            <td class="customer-code-cell">${getCustomerCodeDisplay(sale)}</td>
  ...
  function populateEditCustomerReceiptsTable(customerReceipts) {
    ...
    customerReceipts.forEach((receipt, index) => {
      const row = document.createElement('tr');
      row.innerHTML = `
            <td class="customer-code-cell">${getCustomerCodeDisplay(receipt)}</td>
