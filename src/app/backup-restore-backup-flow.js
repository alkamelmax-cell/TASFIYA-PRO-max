...
function joinDirectoryAndFile(directory, fileName) {
    const cleanDirectory = String(directory || '').trim().replace(/[\\/]+$/, '');
    ...
    try {
        const rows = await ipcRenderer.invoke('db-query', `
            ...
            INNER JOIN (
                ...
                WHERE (category = 'backup' AND setting_key = 'default_backup_path')
                   OR (category = 'database' AND setting_key = 'backup_location')
                GROUP BY category, setting_key
            ) latest
              ON latest.latest_id = s.id
            ORDER BY CASE
                WHEN s.category = 'backup' AND s.setting_key = 'default_backup_path' THEN 0
                WHEN s.category = 'database' AND s.setting_key = 'backup_location' THEN 1
                ELSE 2
            END
        `, []);

        const configuredRow = Array.isArray(rows)
            ? rows.find((row) => row && String(row.setting_value || '').trim())
            : null;
        return configuredRow ? String(configuredRow.setting_value).trim() : '';
    } catch (error) {
        console.warn('⚠️ [BACKUP] تعذر تحميل مجلد النسخ الاحتياطي الافتراضي:', error);
        return '';
    }
}
...
async function handleCreateBackup() {
    ...
    try {
        ...
        const backupFileName = `casher_backup_${new Date().toISOString().split('T')[0]}.json`;
        ...
        const backupPath = await ipcRenderer.invoke('show-save-dialog', {
            ...
            defaultPath: joinDirectoryAndFile(backupDirectory, backupFileName),
            filters: [
