...
function createDetailedAtmReportManagementHandlers(deps) {
  ...
  async function handleShowDetailedAtmReportModal() {
    ...
    try {
      ...
      displayHandlers.handleDetailedReportFeesModeChange();
  ...
  async function handleDetailedBranchFilterChange() {
    logger.log('🏧 [DETAILED-ATM] تحديث فلاتر التقرير حسب الفرع...');
    await dataHandlers.refreshDetailedAtmDependentFilters();
  }
  ...
  return {
    ...
    handleDetailedBranchFilterChange,
    ...
    handleDetailedReportFeesModeChange: async () => {
      await dataHandlers.persistDetailedAtmFeesModeSelection();
      displayHandlers.handleDetailedReportFeesModeChange();
    },
