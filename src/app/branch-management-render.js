function createBranchManagementRenderHandlers(context) {
  ...
  function updateBranchesTable(branches) {
    ...
    if (branches.length === 0) {
      tableBody.innerHTML = '<tr><td colspan="9" class="text-center">لا توجد فروع مسجلة</td></tr>';
      return;
    }
    ...
    tableBody.innerHTML = branches.map((branch) => `
      <tr>
          ...
          <td><span class="badge bg-info text-dark">${branch.customer_code_prefix || '-'}</span></td>
