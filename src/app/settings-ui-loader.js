...
function createSettingsUiLoader(deps) {
  ...
  function normalizeThemeSelection(theme) {
    ...
    if (normalizedTheme === 'sender') {
      return 'sender-soft';
    }
    return ['light', 'dark', 'sender-soft', 'sender-rich'].includes(normalizedTheme)
      ? normalizedTheme
      : 'light';
  }
  ...
  function applyAutoBackupStatusToUI(settings) {
    const statusElement = document.getElementById('autoBackupStatus');
    if (!statusElement) {
      return;
    }

    const status = String(settings.last_auto_backup_status || '').trim().toLowerCase();
    const error = String(settings.last_auto_backup_error || '').trim();
    const warning = String(settings.last_auto_backup_warning || '').trim();
    const lastBackupAt = formatAutoBackupDateTime(settings.last_auto_backup_at);
    const nextBackupAt = formatAutoBackupDateTime(settings.last_auto_backup_next_at);
    const lastFile = String(settings.last_auto_backup_file || '').trim();

    let message = 'لم يتم تنفيذ نسخة تلقائية بعد';
    let levelClass = 'text-muted';
    if (status === 'failed') {
      message = error ? `آخر محاولة فشلت: ${error}` : 'آخر محاولة للنسخ التلقائي فشلت';
      levelClass = 'text-danger';
    } else if (status === 'success') {
      message = lastBackupAt ? `آخر نسخة ناجحة: ${lastBackupAt}` : 'آخر نسخة تلقائية تمت بنجاح';
      if (nextBackupAt) {
        message += `، الموعد القادم: ${nextBackupAt}`;
      }
      if (lastFile) {
        message += `، الملف: ${lastFile}`;
      }
      levelClass = warning ? 'text-warning' : 'text-success';
    } else if (warning) {
      message = warning;
    ...
    statusElement.className = `form-text auto-backup-status ${levelClass}`;
  }
  ...
  function applyGeneralSettingsToUI(settings) {
    ...
    if (Object.prototype.hasOwnProperty.call(settings, 'system_theme')) {
      ...
      const selectedTheme = normalizeThemeSelection(settings.system_theme);
  ...
  function applyReportsSettingsToUI(settings) {
    ...
    const detailedAtmFeesDefaultModeField = document.getElementById('detailedAtmFeesDefaultMode');
    if (detailedAtmFeesDefaultModeField) {
      detailedAtmFeesDefaultModeField.value = settings.detailed_atm_fees_mode_default || 'without_fees';
    }

    bankFeeUiHelpers.renderBankFeeRules(settings.bank_fee_rules_json || settings.bank_fee_rules || {});
  ...
  function applyBackupSettingsToUI(settings) {
    ...
    applyAutoBackupStatusToUI(settings);
  }
