...
async function ensureRequiredTablesExist() {
    ...
    try {
        ...
        try {
            const branchColumns = await ipcRenderer.invoke('db-query', 'PRAGMA table_info(branches)', []);
            const hasCustomerCodePrefix = Array.isArray(branchColumns)
                && branchColumns.some((column) => column.name === 'customer_code_prefix');
            if (!hasCustomerCodePrefix) {
                await ipcRenderer.invoke('db-run', "ALTER TABLE branches ADD COLUMN customer_code_prefix TEXT DEFAULT ''", []);
                console.log('✅ [RESTORE] تمت إضافة الحقل customer_code_prefix إلى branches');
            }
...
async function normalizeRestoredBranchCustomerCodePrefixes() {
    const branches = await ipcRenderer.invoke(
        'db-query',
        'SELECT id, customer_code_prefix FROM branches ORDER BY id',
        []
    );
...
async function restoreDatabaseData(backupData) {
    ...
    try {
        ...
        try {
            ...
            await normalizeRestoredBranchCustomerCodePrefixes();
