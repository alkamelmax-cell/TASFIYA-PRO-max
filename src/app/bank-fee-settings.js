...
const BANK_FEE_OPERATION_ALIASES = Object.freeze([
  { canonical: 'مدى', aliases: ['مدى', 'مدي', 'mada'] },
  { canonical: 'فيزا', aliases: ['فيزا', 'فيزى', 'visa'] },
...
function getOperationTypeAliases(value) {
  const normalizedValue = normalizeText(value);
  if (!normalizedValue) {
    return [];
  }

  const canonicalValue = normalizeOperationType(normalizedValue);
  const matchedOperation = BANK_FEE_OPERATION_ALIASES.find((entry) => (
    entry.canonical === canonicalValue
    || entry.aliases.some((alias) => doesOperationAliasMatch(normalizedValue, alias))
  ));

  const aliases = matchedOperation
    ? [matchedOperation.canonical, ...matchedOperation.aliases]
    : [canonicalValue];
