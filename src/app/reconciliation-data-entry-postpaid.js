...
function createReconciliationDataEntryPostpaidHandlers(context) {
    ...
    try {
      ...
      if (editingPostpaidSaleIndex >= 0) {
        ...
        await ipcRenderer.invoke(
          'db-run',
          'UPDATE postpaid_sales SET customer_id = ?, customer_name = ?, customer_code = ?, amount = ? WHERE id = ?',
          [identity.customer_id || null, identity.customer_name, identity.customer_code, amount, existingSale.id]
        );
      ...
      const result = await ipcRenderer.invoke(
        ...
        [currentReconciliation.id, identity.customer_id || null, identity.customer_name, identity.customer_code, amount]
      );
  ...
  function editPostpaidSale(index) {
    ...
    if (customerCodeEl) {
      customerCodeEl.value = sale.customer_code || '';
    }
