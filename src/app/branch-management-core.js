...
function createBranchManagementCoreHandlers(context) {
  ...
  function getBranchPrefixField() {
    return doc.getElementById('branchCustomerCodePrefix');
  }

  function suggestPrefixFromBranches(branches = []) {
    return getNextCustomerCodePrefix(
      (Array.isArray(branches) ? branches : []).map((branch) => branch.customer_code_prefix)
    );
  }

  async function suggestNextBranchCustomerCodePrefix() {
    try {
      const branches = await ipc.invoke(
        'db-query',
        'SELECT customer_code_prefix FROM branches ORDER BY id',
        []
      );
      return suggestPrefixFromBranches(branches);
    } catch (error) {
      logger.warn('⚠️ [BRANCHES] تعذر اقتراح رمز كود العملاء التالي:', error);
      return 'C1';
    }
  }

  function applySuggestedBranchPrefix(branches = []) {
    const prefixField = getBranchPrefixField();
    const form = doc.getElementById('branchForm');
    if (!prefixField || (form && form.getAttribute('data-edit-id'))) {
      return;
    }

    const currentPrefix = normalizeCustomerCodePrefix(prefixField.value);
    if (!currentPrefix) {
      prefixField.value = suggestPrefixFromBranches(branches);
    }
  }
  ...
  async function loadBranches() {
    ...
    try {
      ...
      applySuggestedBranchPrefix(branches);
  ...
  async function handleBranchForm(event) {
    ...
    const prefixField = getBranchPrefixField();
    const rawCustomerCodePrefix = prefixField ? prefixField.value : '';
    const suggestedPrefix = editId ? '' : await suggestNextBranchCustomerCodePrefix();
    const customerCodePrefix = normalizeCustomerCodePrefix(rawCustomerCodePrefix || suggestedPrefix);
    ...
    if (!formData.customer_code_prefix) {
      getDialogUtils().showValidationError('يرجى إدخال رمز العملاء للفرع بصيغة C1 أو C2');
      return;
    }

    if (prefixField) {
      prefixField.value = formData.customer_code_prefix;
    }
    ...
    try {
      if (editId) {
        ...
        const result = await ipc.invoke('db-run', `
              UPDATE branches
              SET branch_name = ?, customer_code_prefix = ?, branch_address = ?, branch_phone = ?, reconciliation_formula_id = ?, is_active = ?, updated_at = CURRENT_TIMESTAMP
              WHERE id = ?
          `, [
          formData.branch_name,
          formData.customer_code_prefix,
        ...
        const result = await ipc.invoke('db-run', `
              INSERT INTO branches (branch_name, customer_code_prefix, branch_address, branch_phone, reconciliation_formula_id, is_active)
              VALUES (?, ?, ?, ?, ?, ?)
          `, [
          formData.branch_name,
          formData.customer_code_prefix,
  ...
  function clearBranchForm() {
    ...
    if (prefixField) {
      prefixField.value = '';
      suggestNextBranchCustomerCodePrefix()
        .then((prefix) => {
          if (!doc.getElementById('branchForm')?.getAttribute('data-edit-id') && !prefixField.value) {
            prefixField.value = prefix;
          }
        })
        .catch((error) => logger.warn('⚠️ [BRANCHES] تعذر تحديث رمز كود العملاء المقترح:', error));
    }
  }
  ...
  async function editBranch(branchId) {
    try {
      ...
      const prefixField = getBranchPrefixField();
      if (prefixField) {
        prefixField.value = normalizeCustomerCodePrefix(branch.customer_code_prefix) || '';
      }
