...
function createBackupRestoreSettingsActions(context) {
  ...
  let autoBackupStatusListenerAttached = false;
  ...
  function normalizeAutoBackupStatusLevel(level, payload = {}) {
    const normalized = String(level || '').trim().toLowerCase();
    if (['success', 'warning', 'error', 'info'].includes(normalized)) {
      return normalized;
    }
  ...
  function buildAutoBackupStatusMessage(payload = {}) {
    if (payload.message) {
    ...
    if (payload.success === false) {
      return payload.error
        ? `فشل النسخ التلقائي: ${payload.error}`
        : 'فشل النسخ التلقائي';
    }

    if (payload.created) {
      return payload.backupFilePath
        ? `تم إنشاء نسخة تلقائية: ${payload.backupFilePath}`
        : 'تم إنشاء نسخة احتياطية تلقائية بنجاح';
    }

    if (payload.reason === 'not-due') {
      const lastText = formatAutoBackupDateTime(payload.lastBackupAt);
      const nextText = formatAutoBackupDateTime(payload.nextBackupAt);
      if (lastText && nextText) {
        return `النسخ التلقائي يعمل. آخر نسخة: ${lastText}، الموعد القادم: ${nextText}`;
      }
      if (lastText) {
        return `النسخ التلقائي يعمل. آخر نسخة: ${lastText}`;
      }
      return 'النسخ التلقائي يعمل ولا توجد نسخة مستحقة الآن';
    }
    ...
    if (payload.reason === 'path-missing') {
      return 'النسخ التلقائي مفعل لكن لم يتم تحديد مجلد الحفظ';
    }

    return 'تم فحص حالة النسخ الاحتياطي التلقائي';
  }

  function updateAutoBackupStatusDisplay(payload = {}) {
    const statusElement = getElement('autoBackupStatus');
    if (!statusElement) {
      return '';
    }

    const level = normalizeAutoBackupStatusLevel(payload.level, payload);
    const classByLevel = {
    ...
    const warningText = payload.warning && !String(payload.message || '').includes(payload.warning)
      ? ` - ${payload.warning}`
      : '';
    const message = `${buildAutoBackupStatusMessage(payload)}${warningText}`;

    statusElement.textContent = `آخر حالة النسخ التلقائي: ${message}`;
    statusElement.className = `form-text auto-backup-status ${classByLevel[level] || 'text-muted'}`;
    return message;
  }

  function shouldToastAutoBackupStatus(payload = {}) {
    const trigger = String(payload.trigger || '').trim();
    return !['settings-save', 'backup-frequency-change', 'backup-location-change', 'manual'].includes(trigger);
  }

  function notifyAutoBackupStatusIfNeeded(payload = {}) {
    const level = normalizeAutoBackupStatusLevel(payload.level, payload);
    if (!shouldToastAutoBackupStatus(payload) || (level !== 'error' && level !== 'warning')) {
      return;
    }

    const dialog = typeof getDialogUtils === 'function' ? getDialogUtils() : null;
    if (!dialog) {
      return;
    }

    const message = buildAutoBackupStatusMessage(payload);
    if (level === 'warning' && typeof dialog.showWarningToast === 'function') {
      dialog.showWarningToast(message);
      return;
    }
  ...
  function updateAutoBackupStatusFromCheckResult(result = {}) {
    return updateAutoBackupStatusDisplay(result || {});
  }

  function initializeAutoBackupStatusListener() {
    if (autoBackupStatusListenerAttached || !ipcRenderer || typeof ipcRenderer.on !== 'function') {
      return false;
    }

    ipcRenderer.on('auto-backup-status', (_event, payload = {}) => {
      updateAutoBackupStatusDisplay(payload || {});
      notifyAutoBackupStatusIfNeeded(payload || {});
    });
    autoBackupStatusListenerAttached = true;
    return true;
  }
  ...
  async function runImmediateAutoBackupCheckIfEnabled(autoBackup, backupLocation, source = 'settings-update') {
    ...
    if (!frequency || frequency === 'disabled') {
      const disabledResult = { success: true, skipped: true, reason: 'disabled' };
      updateAutoBackupStatusFromCheckResult(disabledResult);
      return disabledResult;
    }

    if (!backupPath) {
      const missingPathResult = {
        success: false,
        skipped: true,
        reason: 'path-missing',
        error: 'تم تفعيل النسخ الاحتياطي التلقائي بدون تحديد مجلد الحفظ'
      };
      updateAutoBackupStatusFromCheckResult(missingPathResult);
    ...
    try {
      ...
      if (!result || typeof result !== 'object') {
        const unknownResult = { success: true, skipped: true, reason: 'unknown-response' };
        updateAutoBackupStatusFromCheckResult(unknownResult);
        return unknownResult;
      }
      if (result.success === false) {
        updateAutoBackupStatusFromCheckResult(result);
        return result;
      }
      ...
      updateAutoBackupStatusFromCheckResult(errorResult);
      return errorResult;
    }
  }
...
الحالة: ${reconciliation.status === 'completed' ? 'مكتملة' : 'مسودة'}${additionalInfo}
  ...
  async function handleSelectBackupLocation() {
    try {
      ...
      const currentBackupLocation = (getElement('backupLocation')?.value || '').trim();

      const result = await ipcRenderer.invoke('select-directory', {
        title: 'اختر مجلد النسخ الاحتياطي',
        defaultPath: currentBackupLocation
      });
  ...
  return {
    ...
    initializeAutoBackupStatusListener,
    updateAutoBackupStatusFromCheckResult
  };
}
