...
function createReconciliationDataEntryCustomerHandlers(context) {
  ...
  const resolveCustomerIdentity = context.resolveCustomerIdentity || (async ({ customerName, customerCode }) => ({
    customer_id: null,
  ...
  async function handleCustomerReceipt(event) {
    ...
    const customerCode = document.getElementById('customerReceiptCode')?.value || '';
    ...
    try {
      ...
      if (editingCustomerReceiptIndex >= 0) {
        ...
        await ipcRenderer.invoke(
          'db-run',
          'UPDATE customer_receipts SET customer_id = ?, customer_name = ?, customer_code = ?, amount = ?, payment_type = ? WHERE id = ?',
          [identity.customer_id || null, identity.customer_name, identity.customer_code, amount, paymentType, existingReceipt.id]
        );
      ...
      const result = await ipcRenderer.invoke(
        'db-run',
        'INSERT INTO customer_receipts (reconciliation_id, customer_id, customer_name, customer_code, amount, payment_type) VALUES (?, ?, ?, ?, ?, ?)',
        [currentReconciliation.id, identity.customer_id || null, identity.customer_name, identity.customer_code, amount, paymentType]
      );
  ...
  function editCustomerReceipt(index) {
    ...
    const customerCodeEl = document.getElementById('customerReceiptCode');
    ...
    if (customerCodeEl) {
      customerCodeEl.value = receipt.customer_code || '';
    }
