function createDetailedAtmReportDisplayHandlers(context) {
  ...
  function isFeesModeEnabled() {
    return doc.getElementById('detailedFeesMode')?.value === 'with_fees';
  }

  function getDetailedReportColumnCount() {
    return isFeesModeEnabled() ? 12 : 9;
  }

  function updateDetailedReportTableHead() {
    const thead = doc.getElementById('detailedAtmReportTableHead');
    if (!thead) {
      return;
    }

    if (isFeesModeEnabled()) {
      thead.innerHTML = `
        <tr>
          ...
          <th>رقم الحساب</th>
          <th>البنك</th>
          <th>المبلغ قبل الرسوم</th>
          <th>الرسوم البنكية</th>
          <th>ضريبة الرسوم</th>
          <th>المبلغ بعد الرسوم</th>
          <th>الكاشير</th>
          <th>رقم التصفية</th>
        </tr>
      `;
  ...
  function displayDetailedReportSummary() {
    ...
    const feesModeEnabled = isFeesModeEnabled();
    ...
    const minAmount = Math.min(...data.map((item) => Number(item.amount) || 0));
    const totalGrossAmount = data.reduce((sum, item) => sum + (Number(item.gross_amount ?? item.amount) || 0), 0);
    const totalFeeAmount = data.reduce((sum, item) => sum + (Number(item.fee_amount) || 0), 0);
    const totalFeeVatAmount = data.reduce((sum, item) => sum + (Number(item.fee_vat_amount) || 0), 0);
    const totalNetAmount = data.reduce((sum, item) => sum + (Number(item.net_amount ?? item.amount) || 0), 0);
    const avgNetAmount = totalNetAmount / data.length;
    ...
    const summaryHtml = feesModeEnabled
      ? `
        ...
        <div class="col-md-2"><div class="card bg-success text-white"><div class="card-body text-center"><h6 class="card-title">قبل الرسوم</h6><h5>${formatCurrency(totalGrossAmount)}</h5></div></div></div>
        <div class="col-md-2"><div class="card bg-warning text-white"><div class="card-body text-center"><h6 class="card-title">الرسوم البنكية</h6><h5>${formatCurrency(totalFeeAmount)}</h5></div></div></div>
        <div class="col-md-2"><div class="card bg-danger text-white"><div class="card-body text-center"><h6 class="card-title">ضريبة الرسوم</h6><h5>${formatCurrency(totalFeeVatAmount)}</h5></div></div></div>
        <div class="col-md-2"><div class="card bg-info text-white"><div class="card-body text-center"><h6 class="card-title">بعد الرسوم</h6><h5>${formatCurrency(totalNetAmount)}</h5><small>المتوسط: ${formatCurrency(avgNetAmount)}</small></div></div></div>
  ...
  function displayDetailedReportTable() {
    ...
    if (pageData.length === 0) {
      tbody.innerHTML = `
            <tr><td colspan="${getDetailedReportColumnCount()}" class="text-center text-muted py-4">لا توجد عمليات تطابق المعايير المحددة</td></tr>
        `;
      return;
    }
    ...
    pageData.forEach((item) => {
      ...
      row.innerHTML = feesModeEnabled ? `
            ...
            <td class="text-end fw-bold">${item.formatted_gross_amount || item.formatted_amount}</td>
            <td class="text-end text-warning-emphasis">${item.formatted_fee_amount || formatCurrency(item.fee_amount || 0)}</td>
            <td class="text-end text-danger">${item.formatted_fee_vat_amount || formatCurrency(item.fee_vat_amount || 0)}</td>
            <td class="text-end text-success fw-semibold">${item.formatted_net_amount || formatCurrency(item.net_amount || 0)}</td>
    ...
    if (!searchTerm) {
      ...
      state.filteredDetailedReportData = state.currentDetailedReportData.filter((item) => {
        if (!isNaN(searchAmount)) {
          return (
            Number(item.amount) === searchAmount
            || Number(item.gross_amount) === searchAmount
            || Number(item.fee_amount) === searchAmount
            || Number(item.fee_vat_amount) === searchAmount
            || Number(item.net_amount) === searchAmount
          );
        }
        return (
          ...
          (item.gross_amount ?? '').toString().includes(searchTerm) ||
          (item.fee_amount ?? '').toString().includes(searchTerm) ||
          (item.fee_vat_amount ?? '').toString().includes(searchTerm) ||
          (item.net_amount ?? '').toString().includes(searchTerm) ||
  ...
  function handleDetailedReportSort() {
    ...
    state.filteredDetailedReportData.sort((a, b) => {
      switch (sortValue) {
        ...
        case 'amount_desc':
          return parseFloat(b.gross_amount ?? b.amount) - parseFloat(a.gross_amount ?? a.amount);
        case 'amount_asc':
          return parseFloat(a.gross_amount ?? a.amount) - parseFloat(b.gross_amount ?? b.amount);
        case 'atm_name':
  ...
  function handleDetailedReportFeesModeChange() {
    if (!Array.isArray(state.currentDetailedReportData) || state.currentDetailedReportData.length === 0) {
      updateDetailedReportTableHead();
      return;
    }
  ...
  return {
    ...
    handleDetailedReportFeesModeChange,
