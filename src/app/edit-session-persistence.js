function createEditSessionPersistence(deps) {
  ...
  const comparableFieldsByType = {
    ...
    postpaidSales: ['customer_id', 'customer_code', 'customer_name', 'amount'],
    customerReceipts: ['customer_id', 'customer_code', 'customer_name', 'amount', 'payment_type'],
  ...
  async function insertUpdatedRecords(data) {
    ...
    for (const sale of data.postpaidSales || []) {
      ...
      await ipcRenderer.invoke(
        'db-run',
        'INSERT INTO postpaid_sales (reconciliation_id, customer_id, customer_name, customer_code, amount, is_modified) VALUES (?, ?, ?, ?, ?, ?)',
        [data.reconciliationId, sale.customer_id || null, sale.customer_name, sale.customer_code || '', sale.amount, isModified]
      );
    }
    ...
    for (const receipt of data.customerReceipts || []) {
      ...
      await ipcRenderer.invoke(
        'db-run',
        'INSERT INTO customer_receipts (reconciliation_id, customer_id, customer_name, customer_code, amount, payment_type, is_modified) VALUES (?, ?, ?, ?, ?, ?, ?)',
        [data.reconciliationId, receipt.customer_id || null, receipt.customer_name, receipt.customer_code || '', receipt.amount, receipt.payment_type || 'نقدي', isModified]
      );
    }
