  ...
  async function handleNewReconciliation(event) {
    ...
    try {
      ...
      if (windowObj.pendingReconciliationData && windowObj.appAPI) {
        ...
        const totalCashFromRequest = parseAmount(pData.total_cash ?? pData.totalCash);
        const totalBankFromRequest = parseAmount(pData.total_bank ?? pData.totalBank);
        ...
        if (cashBreakdown.length === 0 && totalCashFromRequest > 0) {
          await windowObj.appAPI.addCashReceipt(totalCashFromRequest, 1);
        ...
        } else if (totalBankFromRequest > 0) {
          await windowObj.appAPI.addDetailedBankReceipt(
            'من طلب ويب قديم',
            'تحويل',
            totalBankFromRequest,
            'settlement'
          );
          usedSummaryFallback = true;
        }
        ...
        if (usedSummaryFallback) {
          ...
          if (dialogUtils && typeof dialogUtils.showInfo === 'function') {
            dialogUtils.showInfo('تم تحميل مجاميع الطلب فقط لعدم توفر التفاصيل الكاملة داخل السجل', 'تفاصيل غير مكتملة');
          } else if (dialogUtils && typeof dialogUtils.showSuccessToast === 'function') {
            dialogUtils.showSuccessToast('تم تحميل مجاميع الطلب فقط لعدم توفر التفاصيل الكاملة داخل السجل');
          }
        }
