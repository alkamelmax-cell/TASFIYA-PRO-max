...
async function handleSelectReportsPath() {
    try {
        ...
        const currentReportsPath = (document.getElementById('reportsPath')?.value || '').trim();

        const result = await ipcRenderer.invoke('select-directory', {
            title: 'اختر مجلد حفظ التقارير',
            defaultPath: currentReportsPath
        });
...
async function applyPrintSettingsRealTime(settings) {
  ...
  async function handleSaveReportsSettings(event) {
    ...
    try {
      ...
      const bankFeeSettings = bankFeeUiHelpers.collectBankFeeSettingsFromDocument();
      ...
      const settings = [
        ...
        { key: 'detailed_atm_fees_mode_default', value: document.getElementById('detailedAtmFeesDefaultMode').value },
        { key: 'bank_fee_rules_json', value: JSON.stringify(bankFeeSettings) }
      ];
  ...
  function handleAddBankFeeRuleRow() {
    bankFeeUiHelpers.appendBankFeeRuleRow();
  }

  function handleBankFeeRulesTableClick(event) {
    ...
    if (action === 'delete-bank-fee-rule') {
      bankFeeUiHelpers.removeBankFeeRuleRow(actionButton);
    }
  }
  ...
  return {
    ...
    handleAddBankFeeRuleRow,
    handleBankFeeRulesTableClick,
