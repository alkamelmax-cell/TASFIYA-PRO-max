...
function createSystemSettingsUiHelpers(context) {
  ...
  const ALLOWED_THEMES = new Set(['light', 'dark', 'sender', 'sender-soft', 'sender-rich']);
...
function normalizeThemeValue(theme) {
    ...
    return ALLOWED_THEMES.has(normalizedTheme) ? normalizedTheme : 'light';
}
...
function applyGeneralSettings(settings) {
    ...
    if (settingsMap.system_theme) {
        const normalizedTheme = normalizeThemeValue(settingsMap.system_theme);
        const systemThemeField = document.getElementById('systemTheme');
        if (systemThemeField) systemThemeField.value = normalizedTheme;
    }
    ...
    if (settingsMap.system_theme) {
        ...
        console.log('🎨 [SETTINGS] تم تطبيق المظهر:', normalizedTheme);
    }
...
function applyReportsSettings(settings) {
    ...
    const fieldMappings = {
        ...
        'detailed_atm_fees_mode_default': 'detailedAtmFeesDefaultMode'
    };
    ...
    bankFeeUiHelpers.renderBankFeeRules(settingsMap.bank_fee_rules_json || settingsMap.bank_fee_rules || {});
}
...
function applyTheme(theme) {
    ...
    console.log('🎨 [THEME] تطبيق المظهر:', normalizedTheme);
    const body = document.body;

    // Remove existing theme classes
    body.classList.remove('theme-light', 'theme-dark', 'theme-auto', 'theme-sender', 'theme-sender-soft', 'theme-sender-rich');
    ...
    switch (normalizedTheme) {
        ...
        case 'sender-rich':
            body.classList.add('theme-sender', 'theme-sender-rich');
            document.documentElement.setAttribute('data-theme', 'sender-rich');
            localStorage.setItem('theme', 'sender-rich');
            console.log('✨ [THEME] تم تطبيق ثيم هوية تصفية الفخم');
            break;

        case 'sender-soft':
        case 'sender':
            body.classList.add('theme-sender');
            body.classList.add('theme-sender-soft');
            document.documentElement.setAttribute('data-theme', 'sender-soft');
            localStorage.setItem('theme', 'sender-soft');
            console.log('🟢 [THEME] تم تطبيق ثيم هوية تصفية الناعم');
            break;
