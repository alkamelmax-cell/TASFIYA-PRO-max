...
function createBackupRestoreRestoreValidationHandlers(context) {
  ...
  async function repairBackupForeignKeyReferences(data) {
    try {
      ...
      const normalizeBranchPrefixes = () => {
        const usedPrefixes = new Set();
        branches
          .slice()
          .sort((a, b) => normalizeId(a && a.id) - normalizeId(b && b.id))
          .forEach((branch, index) => {
            const existingPrefix = normalizeCustomerCodePrefix(branch && branch.customer_code_prefix);
            ...
            if (branch) {
              branch.customer_code_prefix = nextPrefix;
            }
